...
//copy-pasted code from https://stackoverflow.com/questions/12841845/clear-the-windows-7-standby-memory-programmatically
namespace EmptyStandbyList
{
    ...
    public class ClearMemory
    {
        ...
        public static void ClearFileSystemCache(bool ClearStandbyCache)
        {
            ...
            {
                if (MessageBox.Show($"Clear standby memory exception: {exc.Message}", "Error", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) == DialogResult.Cancel)
                {
                    Environment.Exit(1);
                }
            }
